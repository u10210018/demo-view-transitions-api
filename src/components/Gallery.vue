...
const getRandomColor = () => {
  const num = Math.floor(Math.random() * BG_COLOR_LIST.length)
  return BG_COLOR_LIST[num]
}
</script>

<template>
  <div class="grid grid-cols-2 gap-x-3 gap-y-8 md:grid-cols-3">
    <div v-for="item in props.list" :key="item.id" class="block" :style="{ '--primary-color': getRandomColor() }">
      <RouterLink :to="`/detail/${item.id}`" class="group grid aspect-square grid-cols-1 grid-rows-1">
        <div
        ...
        >
          <img :alt="item.name" :src="item.imageUrl" class="h-full w-full object-cover" />
        </div>
      </RouterLink>
      <div class="flex">
        <h3 class="title-underline mt-4 text-h5 font-bold">{{ item.name }}</h3>
      </div>
        ...
        <div class="text-xs">
          ...
          <div class="flex items-center gap-2">
            <Badge class="bg-brand-02">
              <div class="text-xs">{{ item.weight }} 公斤</div>
            </Badge>
            <Badge class="bg-brand-02">
              <div class="text-xs">{{ item.height }} 公分</div>
            </Badge>
          </div>
        </div>
      </div>
